...
import { DiagramAllModule, SymbolPaletteAllModule ,SnappingService, DiagramModule, Diagram, PointPortModel, ConnectorModel, NodeModel, Connector, NodeConstraints, SnapConstraints, DiagramTools, AlignmentOptions, DiagramContextMenu, Snapping} from '@syncfusion/ej2-angular-diagrams';
...
import {  CommandManagerModel, ContextMenuSettingsModel, DiagramComponent, DiagramRegions,  FileFormats, IHistoryChangeArgs, IScrollChangeEventArgs, PageSettingsModel, RulerSettingsModel, ScrollSettingsModel,  SelectorModel, ShapeAnnotationModel,  ZoomOptions } from '@syncfusion/ej2-angular-diagrams';
...
import { UndoRedo, FlowShapeModel,
  SymbolInfo, IDragEnterEventArgs, SnapSettingsModel, MarginModel, TextStyleModel, StrokeStyleModel,
  OrthogonalSegmentModel, Node, PaletteModel,KeyModifiers,Keys,SelectorConstraints,SymbolPalette,PrintAndExport
...
import { DiagramClientSideEvents, DiagramPropertyBinding } from '../scripts/events';
import { NodeProperties, SelectorViewModel } from '../scripts/selector';

import { PaperSize, UtilityMethods } from '../scripts/utilitymethods';
// import { paletteIconClick } from './script/diagram-common';
Diagram.Inject(UndoRedo,DiagramContextMenu,PrintAndExport,Snapping);
...
public palettes: PaletteModel[] = [
  ...
  { id: 'livingRoom', expanded: true, symbols: this.getlivingRoomShapes(), iconCss: 'e-ddb-icons e-flow', title: 'Living Room' },
  { id: 'bathRoom', expanded: true, symbols: this.getBathRoomShapes(), iconCss: 'e-ddb-icons e-flow', title: 'Bath Room' },
];

public setPaletteNodeDefaults(node: NodeModel): void {
  if(node.id === 'Door_close'||  node.id === 'Double_door_close')
  {
    node.width = 25;
    node.height = 25;
  }
  else if(node.id ==='Circle_Study_Table'||node.id ==='Circular_Table_for_Two' ||node.id ==='Circle_Study_Table1' ||node.id ==='Circle_Study_Table2' ||node.id ==='Circle_Study_Table3'){
    node.width = 27;
    node.height = 27;
  }
  else if(node.id ==='Circle_Dining_Table'){
    node.width = 30;
    node.height = 30
  }
  else if(node.id ==='Rectangle_Dining_Table'){
    node.width = 30;
    node.height = 30;
  }
  else if(node.id ==='Oblong_Dining_Table' ||node.id ==='Rectangle_Dining_Table'|| node.id ==='Oval_Dining_Table' ){
    node.width = 48;
    node.height = 30;
  }
  else if(node.id === 'Rectangular_Table_for_Two'){
    node.width = 26;
    node.height = 30;
  }
  else if(node.id === 'Rectangle_Study_Table'  || node.id === 'Rectangle_Study_Table1' )
  {
    node.width = 35;
    node.height = 22;
  }
  else if(node.id ==='Small_Gas_Range')
  {
    node.width = 50;
    node.height =25;
  }
  else if(node.id === 'Large_Gas_Range' ||node.id ==='Large_Gas_Range1'  )
  {
    node.width = 60;
    node.height = 25;
  }
  else if(node.id ==='Refrigerator' ||node.id ==='Water_Cooler')
  {
    node.width = 25;
    node.height = 25;
  }
  else if(node.id ==='Double_bed' ||node.id ==='Double_bed1')
  {
    node.width = 36;
    node.height = 30;
  }
  else if(node.id ==='Single_bed' ||node.id ==='Single_bed1')
  {
    node.width = 19;
    node.height = 30;
  }
  else if(node.id ==='Book_Case' )
  {
    node.width = 40;
    node.height = 20;
  }
  else if(node.id ==='Warddrobe1'){
    node.width = 34;
    node.height = 30;
  }
  else if(node.id ==='Warddrobe')
  {
    node.width = 35;
    node.height = 20;
  }
  ...
  else if(node.id ==='Large_Plant' ||node.id ==='Small_Plant'||node.id ==='Lamp_light')
  {
    node.width = 20;
    node.height =20;
  }
  ...
  else if(node.id ==='Flat_TV' ||node.id ==='Flat_TV1')
  {
    node.width = 65;
    node.height =12;
  }
  ...
  else if(node.id ==='Sofa' ||node.id ==='Double_Sofa')
  {
    node.width = 50;
    node.height =25;
  }
  
  else if(node.id ==='Single_Sofa'|| node.id ==='Couch')
  {
    node.width = 25;
    node.height = 25;
  }
  ...
  else if( node.id ==='Window_Garden' )
  {
    node.width = 60;
    node.height =10;
  }
  else if(node.id ==='L_Room' || node.id ==='Wall_Corner1'){
    node.width = 45;
    node.height =40;
  }
  else if(node.id ==='T_Wall'){
    node.width = 40;
    node.height = 30;
  }
  else if(node.id ==='Room' || node.id ==='T_Room'  || node.id ==='Wall_Corner'||  node.id ==='Elevator')
  {
    node.width = 45;
    node.height =38;
  }
  ...
  else if(node.id === 'Toilet1' || node.id === 'Toilet2'){
      node.width = 27; 
      node.height = 35;
  }
  else if(node.id === 'Corner_Shower'|| node.id === 'Shower'){
    node.width = 30; 
    node.height = 35;
  }
  else if(node.id === 'Wash_Basin1'){
    node.width = 25;
    node.height = 35;
  }
  else if(node.id === 'Wash_Basin2' || node.id === 'Wash_Basin3'|| node.id === 'Wash_Basin5' || node.id === 'Wash_Basin6'){
    node.width = 24; 
    node.height = 22;
  }
  else if(node.id === 'Double_Sink4' || node.id === 'Double_Sink'|| node.id === 'Double_Sink1' || node.id === 'Double_Sink2'){
    node.width = 48; 
    node.height = 22;
  }
  else if(node.id === 'Refrigerator'|| node.id === 'Water_Cooler' )
  {
    ...
    else if(node.id === 'Bath_Tub' || node.id === 'Bath_Tub1' || node.id === 'Bath_Tub2' || node.id === 'Bath_Tub3' || node.id === 'Bath_Tub4'){
      node.width = 40;
      node.height =22;
    }
    else {
...
public insert(args:any){
    if (this.selectedItem.diagram.selectedItems.nodes.length > 0) {
      ...
      switch (commandType.toLowerCase()) {
        case "insertlink":
            ...
            if (this.diagram.selectedItems.nodes[0].annotations.length > 0) {
                let annotation: ShapeAnnotationModel = this.diagram.selectedItems.nodes[0].annotations[0];
   ...
   public openUploadBox(isOpen: boolean, extensionType: string): void {
    let defaultUpload: any = document.getElementById("fileupload");
...
public toolbarEditorClick(args:ClickEventArgs): void {
    ...
    switch(item)
    {
        ...
        case 'Select Tool':
            ...
            break;
          case 'Straight' : 
          this.diagram.drawingObject = { type: 'Straight' ,targetDecorator:{ shape: "None"}} ;
...
public enableMenuItems(itemText: string, diagram: Diagram): boolean {
  ...
  if (itemText) {
      ...
      if ((itemText === 'Insert Image' || itemText === 'Insert Link') && (diagram as any)?.selectedItems?.nodes?.length === 0) {
        return true;
      }
      ...
      if (itemText === 'Paste' && (diagram.commandHandler?.clipboardData?.clipObject as any)?.length === 0) {
        return true;
      }
